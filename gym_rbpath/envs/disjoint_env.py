...
import random
from gym_rbpath.envs.nsfnet import NSFNET
import logging

class DisjointPathEnv(gym.Env):
    """A robust path routing environment for OpenAI gym"""

    def __init__(self):
        super(DisjointPathEnv, self).__init__()

        # General variables defining the environment
        self.LINK_BW = 300
        self.NUM_NODES = 14
        self.MAX_NUM_CANDIDATES = 4
        self.net = NSFNET(weight = self.LINK_BW)
        self.NUM_LINKS = self.net.G.number_of_edges()
        self.NUM_ACTIONS = comb(self.MAX_NUM_CANDIDATES, 2)             # nCr = 6, where n = 4 and r = 2
        self.DEMAND_BW = [1, 2, 8, 32, 64]
        self.ONEHOT_NODES = LabelBinarizer().fit_transform(np.arange(0, self.NUM_NODES, 1))  # one-hot encode the nodes
        self.demand_src = 0
        self.demand_dst = 0
        self.demand_bw = 0
        ...
        # Observation
        low = np.zeros(71)
        high_tmp1 = np.append(np.full(28,1), np.asarray([64]))  # src, dst, bw
        high_tmp2 = np.full(self.NUM_LINKS, self.LINK_BW)    # current link state
        high = np.append(high_tmp1, high_tmp2) 
        self.observation_space = spaces.Box(low, high, shape=(71,), dtype=np.int32)
    ...
    def take_action(self, action):             
        ...
        if self.net.link_allock(self.demand_src, self.demand_dst, self.demand_bw, action):
    ...
    def get_state(self):
        """Get the observation"""
        # sample src & dst pair
        sd_pair = random.sample(range(0, self.NUM_NODES), 2)
    ...
    def reset(self):
        ...
        for (u, v, wt) in self.net.G.edges.data('weight'):
            self.net.G[u][v]['weight'] = self.LINK_BW
