...
import random
from gym_rbpath.envs.nsfnet_shortest import NSFNET
import logging
...
class ShortestPathEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.net = NSFNET(weight = self.LINK_BW, num_of_candidates = self.NUM_CANDIDATES)
    ...
    def get_reward(self, act):
        ...
        # penalty how many nodes are shared between primary and alternative paths
        selection = self.net.ACT_TABLE[self.demand_src][self.demand_dst][act]
        pri = set(selection[0])
        alt = set(selection[1])
    ...
    def get_state(self):
        ...
        ob = demand.tolist() + link_state + self.net.COM_LINK_TABLE[self.demand_src][self.demand_dst]
