...
class NSFNET(object):
    ...
    def __init__(self, weight):
        ...
        self.ACT_TABLE = self.action2path()    # ACT_TABLE[src][dst][action] = [ pri_path, alt_path ]
    ...
    def candidates(self):
        ...
        for src in range(0, self.NUM_NODES):
            for dst in range(0, self.NUM_NODES):
                paths = list(nx.node_disjoint_paths(self.G, src, dst))
                path_dic[src][dst] = paths
        return path_dic
    ...
    def action2path(self):
        ...
        for src in range(0, self.NUM_NODES):
            for dst in range(0, self.NUM_NODES):
                ...
                if len(self.CANDIDAT[src][dst]) == 2:
                    the_only_pair = acts[0]
                    tmp = []
                    for i in range(0,6):
                        tmp.append(the_only_pair)
                    act_table[src][dst] = tmp
                elif len(self.CANDIDAT[src][dst]) == 3:
                    for i in range(0,3):
                        acts.append(acts[i])
                    act_table[src][dst] = acts
                elif len(self.CANDIDAT[src][dst]) == 4:
                    act_table[src][dst] = acts
